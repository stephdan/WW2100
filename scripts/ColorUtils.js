ColorUtils = (function() {
	...
	my.rgbArrayToString = function(rgb){
		if(!isNaN(rgb[3])) {
	...
	my.arrayOfRGBArraysToArrayOfRGBStrings = function(arrayOfRGBArrays) {
		var arrayOfRGBStrings = [],
			i, rgbArray;
		for(i = 0; i < arrayOfRGBArrays.length; i += 1) {
			arrayOfRGBStrings.push(my.rgbArrayToString(arrayOfRGBArrays[i]));
		}
		return arrayOfRGBStrings;
	};
	
	// TODO allow string arguments for colors, aka "rgb(000,000,000)"
	my.getColorRamp = function(c1, c2, breaks, returnDataType) {
		var ramp = [c1], 
			i, b, k;
		k = 1 / (breaks - 1);
		b = k;
		...
		if(returnDataType === "string") {
			return my.arrayOfRGBArraysToArrayOfRGBStrings(ramp);
		}
		return ramp;
	};
